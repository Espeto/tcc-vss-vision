...
void helpers::createImageFile(cv::Mat &image, int i, std::string fileName)  {
    std::ostringstream fname;

    fname << "../Images/outputs/" << fileName << "_" << i << "_.png";
    cv::imwrite(fname.str(), image);
}

void helpers::drawObject(int x, int y, cv::Scalar color, cv::Mat &dst)
{
    cv::circle(dst, cv::Point(x, y), 5, color, cv::FILLED);
